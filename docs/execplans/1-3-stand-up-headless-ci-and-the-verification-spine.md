...
- Decision (third review round): ADR 007's versioning section gains the rule
  that an added field must carry `#[serde(default)]`, and names both
  directions of compatibility explicitly. Rationale: the record said a minor
  bump is additive without saying what makes it so. Without the default,
  `serde` requires the new field and every recording predating the bump stops
  loading — a whole-corpus break dressed as an additive change. The
  `FormatVersion` rustdoc already stated the rule, so the normative document
  was the one out of step. Sharper here than in ADR 006 because scene fixtures
  are regenerated by `make scenes` and are current by construction, while
  recordings are never regenerated. Date/Author: 2026-09-15, implementor.
- Decision (third review round): explicit `version` requirements are not added
  to the path-only workspace dependencies. Rationale: `thysalion-presentation`
  and `thysalion-harness` have been path-only since roadmap 1.1, nothing
  publishes these crates (`thysalion-test-support` is `publish = false`, and
  the root package depends on none of them), and adding a literal version
  creates a manual bump to keep in step with each member's
  `version.workspace = true`. Consistency with the four existing entries is
  worth more than satisfying a rule aimed at external dependencies. Raised for
  the maintainer in the pull request rather than settled unilaterally.
  Date/Author: 2026-09-15, implementor.
...
A third review round found one more class of defect worth naming, because it
is the same shape as the second round's: a *documented command that does not
run*. The acceptance criterion in this plan spelled `grep -rn "a|b"` without
`-E`, which matches nothing — and for a criterion phrased as "matches only
under `crates/test-support/src/`", matching nothing reads as success. A
verification step that cannot fail is the same hazard as a test that cannot
fail, and it hid in prose rather than in code. The command was verified by
running it, but with the escaping the shell needed rather than the escaping
...
2. `grep -rEn "struct BevyHarness|struct LoaderHarness" crates/` matches
   exactly two lines, both under `crates/test-support/src/`. The `-E` is
   load-bearing: without it `grep` reads the alternation literally and matches
   nothing, so the criterion would appear to pass by finding no duplicates
   anywhere — including the ones it exists to detect.
