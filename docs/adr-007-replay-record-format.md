...
A minor bump is only additive if the added field is *decodable from records
that predate it*, so an added field must carry `#[serde(default)]`. Without it,
`serde` requires the field to be present and every recording made before the
bump stops loading — a whole-corpus break dressed as an additive change, and
the exact failure the range rule exists to prevent. A change that cannot be
made decodable this way is not a minor bump, whatever it looks like in the
Rust source: it is a major one. This is ADR 006's rule for scene documents,
restated here because a corpus makes it sharper — scene fixtures are
regenerated by `make scenes` and are current by construction, while recordings
are never regenerated, so an old record is the only kind of artefact that can
notice.

The two directions of compatibility are therefore handled by different
mechanisms, and it is worth naming both. A *new* reader meeting an *old*
record relies on `#[serde(default)]`. An *old* reader meeting a *new* record
relies on the version probe: the record declares a higher minor, and the
reader refuses it as an unsupported version rather than tripping over the
unknown field that `deny_unknown_fields` would otherwise reject with a
misleading message.
